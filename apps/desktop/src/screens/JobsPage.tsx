...
export function JobsPage({ quickJobs, topGearSessions, selected, onSelect, onQuickJob, onTopGearSession, onResult }: {
  ...
  useEffect(() => {
    if (quick?.state === "succeeded") onResult({ kind: "quick", jobId: quick.id });
  }, [onResult, quick?.id, quick?.state]);
  useEffect(() => {
...
function QuickJobDetail({ job, busy, error, onAction, onResult, t }: { job: JobView; busy: boolean; error: string | null; onAction: (kind: "cancel" | "retry") => Promise<void>; onResult: () => void; t: TFunction }) {
  ...
  return <section className="job-card run-detail-card" aria-live="polite"><div className="job-title"><div><span>{t("jobsPage.job", { id: job.id })}</span><strong>{t(`jobsPage.${job.state}`)}</strong></div><CheckCircle2 aria-hidden="true" size={28} /></div><RunMetadata job={job} t={t} /><div className="progress-track" role="progressbar" aria-label={t("jobsPage.progress", { done: job.succeededBatches, total })} aria-valuemin={0} aria-valuemax={total} aria-valuenow={job.succeededBatches}><span style={{ width: `${total ? job.succeededBatches / total * 100 : 0}%` }} /></div><p className="muted">{t("jobsPage.progress", { done: job.succeededBatches, total })}</p>{job.failure || error ? <div className="inline-error" role="alert"><strong>{t("jobsPage.diagnostic")}</strong><code>{error ?? job.failure}</code></div> : null}<Diagnostics job={job} t={t} /><ul className="attempt-list">{job.attempts.map((attempt) => <li key={attempt.id}><span>{t("jobsPage.attempt", { sequence: attempt.sequence })}</span><strong>{t(`jobsPage.${attempt.state}`)}</strong><small>{formatDuration((attempt.finishedUnixMillis ?? Date.now()) - attempt.startedUnixMillis)}</small>{attempt.cacheHit ? <small>{t("jobsPage.cacheHit")}</small> : null}{attempt.failure ? <code>{attempt.failure}</code> : null}</li>)}</ul><RunActions state={job.state} busy={busy} onCancel={() => void onAction("cancel")} onRetry={() => void onAction("retry")} onResult={onResult} t={t} /></section>;
}
...
function GearJobDetail({ session, busy, error, onAction, onResult, t }: { session: TopGearSessionView; busy: boolean; error: string | null; onAction: (kind: "cancel" | "retry") => Promise<void>; onResult: () => void; t: TFunction }) {
  ...
  return <section className="job-card run-detail-card" aria-live="polite"><div className="job-title"><div><span>{t("topGear.session", { id: session.id })}</span><h2>{t(`topGear.stage_${session.stage}`)}</h2></div><Layers3 aria-hidden="true" size={28} /></div><RunMetadata job={session.currentJob} started={session.createdUnixMillis} t={t} /><div className="optimizer-current-state"><span className={`status-dot ${active(state) ? "status-dot-active" : ""}`} aria-hidden="true" /><strong>{t(`jobsPage.${state}`)}</strong><span>{t(messageKey)}</span></div><ol className="optimizer-stages" aria-label={t("topGear.stageProgress")}>{stages.map((stage, index) => { const step = index < current || session.stage === "complete" ? "complete" : index === current ? "current" : "upcoming"; return <li data-state={step} aria-current={step === "current" ? "step" : undefined} key={stage}><span>{index + 1}</span><div><strong>{t(`topGear.stage_${stage}`)}</strong><small>{t(`topGear.step_${step}`)}</small></div></li>; })}</ol><div className={`progress-track ${active(state) ? "progress-track-indeterminate" : ""}`} role="progressbar" aria-label={t("topGear.progress", { done: session.completedExecutions, total: session.totalExecutions })} aria-valuemin={0} aria-valuemax={session.totalExecutions} aria-valuenow={active(state) ? undefined : session.completedExecutions}><span style={active(state) ? undefined : { width: `${session.totalExecutions ? session.completedExecutions / session.totalExecutions * 100 : 0}%` }} /></div><p className="muted">{t("topGear.progress", { done: session.completedExecutions, total: session.totalExecutions })}</p>{diagnostic ? <div className="inline-error" role="alert"><strong>{t("jobsPage.diagnostic")}</strong><code>{diagnostic}</code></div> : null}<Diagnostics job={session.currentJob} t={t} /><div className="button-row">{active(state) ? <button className="secondary-button" disabled={busy} type="button" onClick={() => void onAction("cancel")}><CircleStop aria-hidden="true" size={18} />{t("jobsPage.cancel")}</button> : null}{["failed", "canceled", "interrupted"].includes(state) || session.pipelineFailure ? <button className="secondary-button" disabled={busy} type="button" onClick={() => void onAction("retry")}><RotateCcw aria-hidden="true" size={18} />{t("jobsPage.retry")}</button> : null}{session.stage === "complete" ? <button className="primary-button" type="button" onClick={onResult}><BarChart3 aria-hidden="true" size={18} />{t("jobsPage.viewResult")}</button> : null}</div></section>;
}

const formatDuration = (milliseconds: number) => `${Math.max(0, Math.floor(milliseconds / 60_000))}m ${Math.max(0, Math.floor(milliseconds / 1_000) % 60)}s`;
function RunMetadata({ job, started = job.createdUnixMillis, t }: { job: JobView; started?: number; t: TFunction }) {
  return <dl className="run-metadata"><div><dt>{t("jobsPage.character")}</dt><dd>{job.profile.name} · {job.profile.specialization}</dd></div><div><dt>{t("jobsPage.elapsed")}</dt><dd>{formatDuration((job.state === "running" ? Date.now() : job.updatedUnixMillis) - started)}</dd></div><div><dt>{t("jobsPage.cpu")}</dt><dd>{t(`quick.${job.cpuPreset}`)} · {t("jobsPage.threads", { count: job.settings.threads })}</dd></div><div><dt>{t("jobsPage.settings")}</dt><dd>{job.settings.fightStyle} · {job.settings.desiredTargets} · {job.settings.iterations.toLocaleString()}</dd></div></dl>;
}
function Diagnostics({ job, t }: { job: JobView; t: TFunction }) {
  return job.recentDiagnostics.length ? <details className="run-diagnostics"><summary>{t("jobsPage.recentDiagnostics")}</summary><pre>{job.recentDiagnostics.join("\n\n")}</pre></details> : null;
}

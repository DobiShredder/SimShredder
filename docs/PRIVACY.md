...
1. `http://downloads.simulationcraft.org/nightly/?C=M;O=D` to find the newest official SimulationCraft artifact for the current supported operating system and architecture.
2. `http://downloads.simulationcraft.org/nightly/<exact-discovered-file>` to check and, only after user consent, download that artifact.
...
The SimulationCraft clients reject redirects and bound response sizes and timeouts. SimShredder accepts only the exact official host, nightly path, supported target filename pattern, and size announced by the official server. It records the downloaded artifact's SHA-256 locally and verifies the executable before activation. Because the upstream nightly endpoint is HTTP, this protects local cache integrity and limits unexpected input but does not cryptographically authenticate the server or artifact. SimulationCraft is separate software and is downloaded directly from its official server, not redistributed by SimShredder.

All `0.x` test versions disable remote WoW icon providers and do not display WoW artwork. Built-in semantic placeholders and locally rendered details keep all simulation flows available offline. There is no Blizzard API, advertising, telemetry, analytics, or third-party tooltip request in `0.x`. The Profile screen displays **Reload from Armory** as a disabled control until the `1.0.0` credential broker and privacy contract are approved, so clicking it cannot start a request or open the replacement flow. The optional Wowhead browser action sends only the selected numeric item or spell ID in the page path; it does not include a character name, realm, profile, result, or tracking parameter. A future `1.0.0` icon or Armory provider requires a separate privacy and release audit before activation.

The operating system and contacted servers necessarily receive ordinary connection metadata such as IP address, time, available TLS information, and HTTP headers. The HTTP listing and artifact requests are not encrypted, so the requested public filename and downloaded bytes can be observed or modified in transit. The locally recorded hash detects later cache modification but cannot establish the original artifact's authenticity. SimShredder does not add a character name, realm, profile, result, or installation identifier to these requests.

## Local data

Profiles, the last and previous per-character input snapshots, favorites, generated `.simc` input, job state, logs, JSON, HTML, results, managed SimulationCraft versions, runtime metadata, and icon cache metadata stay on the current user's device. Character profiles are stored by atomic replacement in `character-profiles.json` inside the selected workspace. The default control root is `~/Library/Application Support/SimShredder` on macOS and `%LOCALAPPDATA%\SimShredder` on Windows. Default exports use the user's `Documents/SimShredder Exports` directory.
